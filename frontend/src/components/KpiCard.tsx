...
import { AlertCircle, AlertTriangle, CheckCircle2, TrendingUp, TrendingDown, Minus, FileText, ChevronRight } from 'lucide-react';
      ...
      {card.subtitle && (
        <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-3">
          ...
          {(card.status === 'critical' || card.status === 'warning') && (
            <a
              href="#esteira"
              className="shrink-0 inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 hover:border-emerald-300 transition shadow-xs whitespace-nowrap group"
              title="Ver anomalia correlacionada e diagnóstico na Esteira de Decisão"
            >
              <FileText className="w-3.5 h-3.5 text-emerald-600 group-hover:scale-110 transition-transform" />
              <span>Ver Diagnóstico</span>
              <ChevronRight className="w-3 h-3 text-emerald-600 opacity-60 group-hover:translate-x-0.5 transition-transform" />
            </a>
          )}
        </div>
      )}
